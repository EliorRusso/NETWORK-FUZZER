...
void build()
{
	Packets_Sent = fopen("/home/elior/Fuzzer/Mutate/InfoToSend/rawpackets.txt", "a");
	...
	fprintf(Packets_Sent, "******************UDP Packet**********************\n");
	fprintf(Packets_Sent, "\n");
	fprintf(Packets_Sent, "IP Header\n");
	fprintf(Packets_Sent, "   |-IP Version        : %d\n", (unsigned int)iph->version);
	fprintf(Packets_Sent, "   |-IP Header Length  : %d DWORDS or %d Bytes\n", (unsigned int)iph->ihl, ((unsigned int)(iph->ihl)) * 4);
	fprintf(Packets_Sent, "   |-Type Of Service   : %d\n", (unsigned int)iph->TypeOfService);
	fprintf(Packets_Sent, "   |-IP Total Length   : %d  Bytes(Size of Packet)\n", ntohs(iph->TotalLength));
	fprintf(Packets_Sent, "   |-Identification    : %d\n", ntohs(iph->Identification));
	...
	if ((unsigned int)iph->Ttl < 64)
	{
		fprintf(Packets_Sent, "   |-TTL Shouldnt Pass     : %d\n", (unsigned int)iph->Ttl);
	}
	else
	{
		fprintf(Packets_Sent, "   |-TTL      : %d\n", (unsigned int)iph->Ttl);
	}
	fprintf(Packets_Sent, "   |-Protocol : %d\n", (unsigned int)iph->Protocol);
	fprintf(Packets_Sent, "   |-Checksum : %d\n", ntohs(iph->HeaderChecksum));
	fprintf(Packets_Sent, "\nUDP Header\n");
	fprintf(Packets_Sent, "   |-Source Port      : %d\n", ntohs(uh->SourcePort));
	fprintf(Packets_Sent, "   |-Destination Port : %d\n", ntohs(uh->DestinationPort));
	if (ntohs(uh->Length) > 20)
	{
		fprintf(Packets_Sent, "   |-UDP Length Shouldnt Pass: %d\n", ntohs(uh->Length));
	}
	else
	{
		fprintf(Packets_Sent, "   |-UDP Length       : %d\n", ntohs(uh->Length));
	}
	fprintf(Packets_Sent, "   |-UDP Checksum     : %d\n", ntohs(uh->Checksum));
	for(int i =0; i < num; i++){
		fprintf(Packets_Sent, "   |-Field %d     : %d\n", i, b->array[i]);
	}
	fclose(Packets_Sent);
	free(sendbuff);
}
...
int main()
{
	...
	// 	}
	// }
	Packets_Sent = fopen("/home/elior/Fuzzer/Mutate/InfoToSend/rawpackets.txt", "w");
	sendPackets();
}